...
path2 = sys.argv[2] #tmp_binary_folds
split_size_train = float(sys.argv[3]) #0.225
folds = int(sys.argv[4]) #4
  ...
  for i in range(len(files)):
    ...
    if os.path.getsize(source_file) == 0:
      ...
      if i <= int(len(files)*SPLIT_SIZE_TRAIN):
        ...
        copyfile(source_file, dict_files["validation_file_fold4"])
        ...
        copyfile(source_file, dict_files["train_file_fold4"])
      ...
      elif  (i > 3*int(len(files)*SPLIT_SIZE_TRAIN)) & (i <= 4*int(len(files)*SPLIT_SIZE_TRAIN)):
      ...
      print("Hard-coded number of folds of 4") 
