...
in_data = sys.argv[3]
num_classes = 2                              
file_path = "/jmain02/home/J2AD015/axf03/hxm18-axf03/code/simclr_jade/" + in_model + "/" + in_file + "/saved_model"
data_root_train = pathlib.Path(r"/jmain02/home/J2AD015/axf03/hxm18-axf03/images/"+in_data+"/training")
data_root_val = pathlib.Path(r"/jmain02/home/J2AD015/axf03/hxm18-axf03/images/"+in_data+"/validation")
data_root_test = pathlib.Path(r"/jmain02/home/J2AD015/axf03/hxm18-axf03/images/"+in_data+"/testing")
...
for epoch in range(15):
    ...
    if np.array(acc_val_list_in).mean() > acc_val_max:
        y_true_list, prediction_list_a0, prediction_list_a1 = [], [], []
    ...
    for i, features in enumerate(ds_test):
        ...
        if np.array(acc_val_list_in).mean() > acc_val_max:
            print(epoch, np.array(acc_val_list_in).mean())
            y_true_list.append(np.array(y_true).flatten())
            prediction = tf.nn.sigmoid(logits_t).numpy()
            prediction_list_a0.append(np.array(prediction[:, 0]))
            prediction_list_a1.append(np.array(prediction[:, 1]))
    ...
    if np.array(acc_val_list_in).mean() > acc_val_max:
        acc_val_max = np.array(acc_val_list_in).mean()
...
precision, recall, thresholds = precision_recall_curve(np.concatenate(y_true_list).ravel(), np.concatenate(prediction_list_a1).ravel())
dict_ = {'prec': list(precision), 'recall': list(recall)}
df = pd.DataFrame(dict_)
df_name = in_file + "_" + in_data +"_roc.csv"
df.to_csv(df_name)


dict_ = {'y_true': list(np.concatenate(y_true_list).ravel()), 'pred_a1': list(np.concatenate(prediction_list_a1).ravel()), 'pred_a0': list(np.concatenate(prediction_list_a0).ravel())}
df = pd.DataFrame(dict_)
df_name = in_file + "_" + in_data +"prediction_roc.csv"
df.to_csv(df_name)
