...
def parse_images(image_path, y_train):
    image_string = tf.io.read_file(image_path)
    image = tf.image.decode_jpeg(image_string, channels=3)
    image = tf.image.convert_image_dtype(image, tf.float32)
    image = tf.image.resize(image, size=[FLAGS.image_size, FLAGS.image_size])
...
def build_input_fn(builder, global_batch_size, topology, is_training):
  ...
  def _input_fn(input_context):
    ...
    data_root = pathlib.Path(FLAGS.image_dir)
    ...
    if FLAGS.labels == "random":
        all_image_labels = list(np.random.randint(FLAGS.num_of_classes, size=len(all_image_labels)))
    else:
        None
