...
def main(argv):
  ...
  X_train = np.zeros((100,FLAGS.image_size,FLAGS.image_size, 3), dtype=np.float32)
  y_train = np.random.randint(num_classes, size=100)

  #create a dataset object now 
  builder = tf.data.Dataset.from_tensor_slices((X_train,y_train ))
  del X_train, y_train
  
  number_of_imgs = sum([len(files) for r, d, files in os.walk(FLAGS.image_dir)])
  num_train_examples = number_of_imgs
  num_eval_examples = number_of_imgs
